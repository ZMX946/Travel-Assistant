...
from models.test_llm import comprehensive_langchain_test
...
class LLMFactory:
    ...
    def get_llm(model=None, temperature=None):
        if model is None or temperature is None:
            raise ValueError("必须指定“model”和“温度”.")
        ...
        #     return ChatTongyi(model=model, temperature = temperature, streaming=True)
        #
        # raise ValueError(f"model {model} 不支持.")

        # 匹配阿里云通义千问模型，未显示传递的话，api_key会自动读取系统环境变量"DASHSCOPE_API_KEY"，base_url由库代码自动指定
        # if model.startswith('qwen'):
        #     return ChatTongyi(model=model, temperature = temperature, streaming=True)
        # 匹配其他兼容OpenAI接口的模型
        load_dotenv()
        result = comprehensive_langchain_test(api_base_url=os.getenv("API_BASE_URL"), api_key=os.getenv("LLM_API_KEY"), model_name=model)
        if result:
            return ChatOpenAI(model=model, temperature=temperature, streaming=True,
...
代码逻辑：
    ...
    如果model以"qwen"开头(例如"qwen-3.5-turbo"或 "qwen-4")，则使用ChatTongyi类创建一个通义千问的聊天模型实例。
    创建实例时，传入的参数包括：
        model: 模型名称。
